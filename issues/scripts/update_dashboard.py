...
def update_dashboard(module_details_json):
    ...
    for module in module_details_json['modules']:
        row = ("|[" + module['name'].split('-')[-1] + "](" + BALLERINA_ORG_URL + module['name'] + ")| " +
               "[![Issues](" + GITHUB_BADGE_URL + "issues-search/" + BALLERINA_ORG_NAME + "/ballerina-standard-library?"
               ...
               "[![Improvements](" + GITHUB_BADGE_URL + "issues-search/" + BALLERINA_ORG_NAME + "/ballerina-standard-library?"
               + get_query(module, "Improvement") + ")](" + get_link(module, "Improvement") + ")| " +
               "[![NewFeatures](" + GITHUB_BADGE_URL + "issues-search/" + BALLERINA_ORG_NAME + "/ballerina-standard-library?"
               + get_query(module, "NewFeature") + ")](" + get_link(module, "NewFeature") + ")| " +
               "[![Tasks](" + GITHUB_BADGE_URL + "issues-search/" + BALLERINA_ORG_NAME + "/ballerina-standard-library?"
...
def get_all_issue_query(module):
    ...
    try:
        data = url_open_with_retry("https://api.github.com/repos/ballerina-platform/ballerina-standard-library/" +
                                   "issues?state=open&labels=area/security,module/" + get_module_short_name(module_name))
...
def get_query(module, type):
    ...
    try:
        data = url_open_with_retry("https://api.github.com/repos/ballerina-platform/ballerina-standard-library/" +
                                   "issues?state=open&labels=area/security,Type/" + type + ",module/" + get_module_short_name(module_name))
        json_data = json.load(data)
        issue_count = len(json_data)
    except Exception as e:
        print('Failed to get issue details for ' + module_name + ": " + str(e))
...
def get_all_issue_link(module):
    return BALLERINA_ORG_URL + "/ballerina-standard-library/issues?q=is%3Aopen+label%3Aarea%2Fsecurity+label%3Amodule%2F" + get_module_short_name(module['name'])


def get_link(module, type):
    return BALLERINA_ORG_URL + "/ballerina-standard-library/issues?q=is%3Aopen+label%3Aarea%2Fsecurity+label%3AType%2F" + type + "+label%3Amodule%2F" + get_module_short_name(module['name'])
