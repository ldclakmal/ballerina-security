...
- The 'API Gateway' passes the new JWT to the 'Order Service', and it needs only trust this STS to accept
  the token as valid. The 'Order Service' validates the signature of the JWT to make sure that it's issued
  by the 'STS' it trusts. Apart from the signature validation, the 'Order Service' also does the issuer and
  audience validation.
