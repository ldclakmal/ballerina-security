...
sed -i 's+http:Response response = check securedEP->get("/foo/bar");+json response = check securedEP->post("/graphql", { "query": "{ greeting }" });+g' jwt/client.bal
sed -i 's+io:println(check response.getTextPayload());+io:println(response);+g' jwt/client.bal
